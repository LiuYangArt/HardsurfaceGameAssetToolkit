...
-   **评估优先开发 (Evaluation-First Development)**: 在任何代码编写之前，精确规划解决方案并确认对任务目标的理解。 [2][<sup>1</sup>](https://www.linkedin.com/pulse/vibe-coding-ai-fabian-best-practices-every-project-fabian-williams--hljke)
-   **专注与精准**: 你是负责高杠杆、生产安全更改的高级工程师。不要即兴发挥、过度设计或偏离任务范围。 [2]

## 2. 任务范围与规划 (Define Scope & Plan) [2]
在开始编写任何代码之前，必须：
-   **明确任务目标**: 首先明确任务范围，确认你对目标的理解。
-   **制定清晰的计划**: 编写清晰的计划，详细说明将涉及哪些Blender功能、Python模块或组件，并解释其原因。在完成并充分推理之前，不要开始实施。
-   **检查可复用性**: 在实施前，检查项目内部的 `CommonFunctions.py` 文件中是否有可复用的功能。优先使用现有功能，避免重复造轮子。
...
## 4. 最小化与精确化更改 (Minimize & Isolate Changes) [2]
...
## 5. 质量保证 (Quality Assurance) [2]
-   **彻底检查**: 仔细审查代码的正确性、与任务范围的一致性以及是否存在副作用。
-   **模式一致性**: 确保你的代码与现有代码库的模式和约定保持一致，避免引入回归。
-   **下游影响**: 明确验证更改是否会对系统的其他部分造成负面影响。
-   **代码质量**: 注重代码风格、性能、可维护性和潜在的安全性问题。 [<sup>2</sup>](https://gist.github.com/mearleycf/5d32bc345c67696928db9ebc987241c7)
