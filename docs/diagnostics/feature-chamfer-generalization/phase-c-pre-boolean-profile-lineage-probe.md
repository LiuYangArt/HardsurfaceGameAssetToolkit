...
这进一步排除了“complete cutter lineage 根本没生成”；尚待区分的是 Exact Boolean 没把这 46 个 Face ID 传给 surviving groove Faces，还是现有 Boundary witness/ledger 只记录了交线一侧而漏掉了这段 direct incidence。Patch 15 的 side-compatible Edge 只能作为几何存在性验证，不能在没有 Face identity 的情况下升级成 owner。下一步必须在 `modifier_apply` 后对 `PHASE_C_CUTTER_FACE_ID_ATTRIBUTE` 做 output Face→Edge incidence census，再与 `_mark_boolean_boundary_witnesses` 的 marked Edge 集合比较；这可以区分 ID 未传播、只传播到非 Boundary groove Face、或 witness/extraction 漏记。继续禁止 nearest、坐标或 Patch 15 fallback。
...
Plan lineage 表显示目标 strand 只有 `[3,4]` 与 `[3,5]` 两个 StripCorrespondence，Rail 也只有 Patch `3/4/5`，没有 Patch 15 rail、correspondence 或 junction-port incidence。因此 Patch 15 / side 1 Edge 即使在 token graph 中恰好填补缺口，也没有现有 Plan 身份授权，不能成为 Go witness。当前可验证结论是：新主路线仍正确 fail-closed，但现有 Plan 的 pre-Boolean profile-neighbor/port 身份不足以解析该遮挡段；继续工作前需要设计一种新的、非几何猜测的 Boolean 前 occlusion transition lineage。不得把 Patch 15、最近 sink、token-only 或单 Pipe当 fallback。
