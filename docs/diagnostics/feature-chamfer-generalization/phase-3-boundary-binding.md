...
## 2026-07-22 source-topology identity 审计

在同一 multi-input Exact 结果的 disposable BMesh 内，删除 groove Faces 前保留
`BMEdge` identity，并额外把 source-local Vertex/Edge ID 写入 POINT/EDGE attribute。
该审计把此前“完全没有 field”的 10 条 Boundary Edge 重新分类为：

- 两个 radius 的 10/10 都携带 source-local EDGE identity，且不是 native
  `Intersecting Edges` 或 cutter-cutter seam；Boolean 会把一条 source Edge 切成多段，
  所以部分结果 Edge 的端点是新交点，但其拓扑来源仍是 source Edge；
- closed result 中这些 Edge 只有 retained source Face 邻接；source Patch 均为 `0`；
- 两个 radius 命中同一组五条 source Edge ID：`1882`、`1913`、`1914`、`2318`、
  `2756`；其中 `1882` / `1913` 是约 `2e-7` 的极短 source Edge；
- 这些 source Edge 都不是 Sharp Edge，也不属于任何 `FeatureStrand`，其 source-local
  ID 经 Exact Boolean 传播到结果 EDGE；因此把它们解释为 Pipe/Rail witness 会制造
  虚假的 owner，不能用最近 Pipe、邻接传播或“所有 Pipe”兜底；
- 旧 probe 把“删除所有非 original Faces 后所有单面 Edge”全部计为 Chamfer
  Boundary，因而把原 source topology defect 错算为未消费 groove Boundary。

artifact：

`/tmp/hst_source_port_map_retry/feature_chamfer_multi_input_tricky_solid_004_r0.010.json`
...
合同冲突。fresh matrix：`/tmp/hst_phase0_reaudit/results.json`。Phase 3 继续
`PROTOTYPE / STOP`。

剩余非 source fragment 的缺口也已缩小到同一合同错误：radius `0.01` 为 `6` 条、
radius `0.03` 为 `9` 条，全部具有唯一 Pipe、compound endpoint token、native
intersection 与 retained Patch `4`。Pipe `6` / `8` 的 plan strand 却都只拥有
`OWNER_PATCH:0` Rail；当前 port fallback 再按 Patch `4` 全局查 Rail，会命中另一条
strand，违反 Pipe→FeatureStrand owner。也就是说，Port token 没丢，错误发生在
`ChamferPlan` 把整个长 strand 的 owner Surface 压成统一 Rail：实际 cutter 在 strand
endpoint 与 Patch `4` 相交，但 plan 没表达“endpoint Port 在相邻 Patch 上的合法
occluded range”。这不是允许忽略 Pipe owner 的理由；下一 slice 必须在 plan 中增加
Port-local owner Patch incidence/range，再由同一 `(Pipe, Port, Patch)` 生成 witness。
