...
只读 maximal-chain pairing 已在 `tricky__solid_004__r0p030` 与 `r0p010` 得到 probe Go；截至扩展 cell 的独立 spec audit 为 `P0=0 / P1=0`。`mixed__extruded_002__r0p030` 仍有 23 个 source Face identity 缺 direct consumer，candidate token graph 也不连成唯一 open chain，因此保持 hard Stop。三表与 raw Boolean output census 已证明：缺口对应 open chain 中连续 `24` 段、`48` 次 expected neighbor 引用（`46` 个唯一 Face signatures）；每个 Face ID 都正确传播到 1 个 groove Face，但这些 Face 全部不与 source 共享 Edge，所以不会也不应进入 Boundary ledger。失败点不是 Boolean 没生成边、Face ID 丢失或 ledger 漏标，而是“profile-neighbor Face 必须直接接触对侧 Boundary”的假设在该遮挡区不成立。Plan 只有 `[3,4]`、`[3,5]` correspondence 和 Patch `3/4/5` rails，没有 Patch 15 rail/correspondence/port incidence，故可见 Patch 15 Edge 不能成为 Go witness。下一步需先设计并审查一种 Boolean 前冻结、非几何猜测的 occlusion transition lineage；正式 Adapter runtime、`FINALIZE`、Phase D/E 仍不在本阶段。
