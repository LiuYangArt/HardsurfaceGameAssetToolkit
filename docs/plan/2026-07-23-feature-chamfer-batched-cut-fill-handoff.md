...
- 同轮 regular 修复：`build_chamfer_strip()` 的 Phase C 调用现把 zero-area 作为 DP step 硬约束，而不是先选出退化最短路再整体拒绝；没有合法单调路径时返回结构化 `NO_MONOTONIC_CORRESPONDENCE_PATH`。定向结果 `305770…` 已从 unresolved 消除，regular strip Faces `698→714`、zero-area `0`，但另一个既有内部 `pipe0 0:1` fragment 因该硬约束显式暴露，unique unresolved 总数仍为 `8`。这不是 Phase C GO；下一步继续处理 pipe2 的 2 个 mismatch、pipe0 的 4 个 fragment、pipe9 的 width/non-monotonic 2 个 component。
- 进一步把现有 signed-width / relative-advance hard guard 纳入 DP path cost（阈值不变），避免先选到必然被后置 guard 拒绝的低成本对应。定向结果新增 2 个合法 regular components，`regular_core_count=23`、Faces `799`、unique unresolved `8→6`；Universe/consumer/order/zero-area 门禁保持通过。剩余为 pipe2 大跳错配 `1`、pipe0 退化/长跨 fragment `3`、pipe9 cross-span width/non-monotonic `2`，继续 Phase C STOP。
