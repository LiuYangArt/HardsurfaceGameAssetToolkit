...
> 目标版本：Blender 5.0+
>
> 目标入口：`hst.prepcadmesh`、`hst.fixcadobj`
>
...
同一套纯 helper 也应集成进 `HST_OT_FixCADObj`。该入口当前正好负责 Merge Vert → 水密检查 → 根据原 split normals 标记 Sharp，Safe Ngon 与它的职责相符；但必须在修改前保存法线参考，并在 Clean/Merge/Safe Ngon 全部完成后恢复 normals，再标记 Sharp。Prepare CAD Mesh 和 Fix CAD Obj 应共享同一条 CAD topology repair pipeline，区别仅是后者不生成 UV。
...
入口：`operators/cad_ops.py:154-197`。

当前顺序：

1. Apply Rotation / Scale；
2. `Mesh.merge_verts_ops()`；
3. 检查开放边界；
4. `mark_sharp_edges_by_split_normal()`。

它比 Prepare CAD Mesh 更适合作为独立 CAD 几何修复入口，Safe Ngon 可以集成。关键顺序必须调整为：修改前 source snapshot → Apply → Merge → Safe Ngon → custom normal restore → mark Sharp → 末端 topology validation。若在 Safe Ngon 之后才保存或恢复 normals，就无法可靠保留 CAD 导入时的 shading/Sharp 信号。

两个 Operator 应调用共享 pipeline，禁止复制两份 Safe Ngon 编排代码：

```text
repair_cad_mesh(mesh, options) -> stats
```

- Fix CAD Obj：运行 topology + normals + Sharp，结束；
- Prepare CAD Mesh：运行同一 pipeline，再继续 Seam / Auto Seam / UV Unwrap；
- 两者默认参数应一致，避免同一 Mesh 因入口不同得到不同拓扑；
- Fix CAD Obj 已经做过的 Mesh 再进入 Prepare 时必须可重复执行且不继续漂移。
...
### Phase 3B：接入 Fix CAD Obj

`HST_OT_FixCADObj` 复用 Phase 3 的共享 pipeline，但不进入 UV 阶段：

1. 入口一次性校验选择与上下文；
2. 对每个 Mesh 创建 source snapshot；
3. Apply transform；
4. Merge by Distance；
5. Safe Ngon；
6. 恢复 custom normals；
7. 根据恢复后的 normals 标记 Sharp；
8. 验证 closed manifold、无 loose geometry、无 zero-area Face；
9. 清理 source，恢复用户上下文并汇总统计。

Fix CAD Obj 不额外运行 `clean_mid_verts()` / `clean_loose_verts()`，除非先用真实 OBJ fixture 证明这是该入口既有目标且不会扩大破坏范围。它与 Prepare 的共用部分应封装为可配置 pipeline，而不是让 Fix CAD Obj 直接调用 `bpy.ops.hst.prepcadmesh()`。

补充回归：Fix CAD Obj 的 Safe Ngon topology、custom normals、Sharp、重复执行、多对象、Multi-user、异常清理，以及“Fix 后再 Prepare”不继续改变 topology。

### Phase 4：Operator 交互

修改 `HST_OT_PrepCADMesh` 与 `HST_OT_FixCADObj` 时遵循项目 Operator 规范：
...
- Fix CAD Obj 运行 Safe Ngon 后正确恢复 normals 并标记 Sharp；
- 同一 Mesh 执行 Fix CAD Obj → Prepare CAD Mesh 时 topology 不继续漂移。
...
2. Fix CAD Obj 默认运行 Merge → Safe Ngon → Normal Restore → Mark Sharp；
3. 两个入口共享同一 topology/normal pipeline，Fix → Prepare 重复执行不会继续改变 topology；
4. 外部 Safe Ngon add-on 未启用时也能独立工作；
5. 不修改 `auto_load.py`；
6. 不留下临时 Object、Mesh、Modifier 或 Attribute；
7. custom normals、Sharp、Seam、UV、Material 和约定 Attributes 通过回归；
8. 结果无新 boundary/non-manifold/zero-area 问题；
9. 多对象、Multi-user、重复执行、异常清理通过；
10. 完整 `python .\tools\run_blender_tests.py` 通过；
11. Blender GUI 中 Undo 与 Adjust Last Operation 通过人工验收；
12. Safe Ngon 来源与 GPL attribution 已记录。
...
3. 定位 `operators/cad_ops.py:16-197`、`utils/mesh_utils.py:27-67,791-895`；
