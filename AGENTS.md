...
### 诊断型几何 Operator 禁止“安全失败”

- 对 Feature Chamfer 及同类需要暴露坏拓扑供排查的几何 Operator，禁止把拓扑质量、边界复杂度、搜索预算、中间合同不满足、自交、开放边、非流形或零面积面作为撤销结果、回滚整个流程或阻止 Operator 完成的理由。
- 有效输入已经建立工作 Mesh 后，即使局部算法无法继续，也必须发布中断瞬间的工作 Mesh，并继续完成所有仍可执行的收尾步骤，例如清理临时对象、保留诊断、隐藏 source、选择并显示结果。只能跳过确实依赖失败步骤且无法执行的局部操作，禁止停止整个 Operator。
- 禁止按错误码逐项维护“允许继续”名单或只修复当前最先出现的报错。修改此类流程时，必须从正式入口到最终发布审计全部异常、`CANCELLED`、回滚和提前返回路径，并在正式入口统一保证整类结构化几何异常不会阻止结果发布。
- 回归必须覆盖未来新增的未知结构化几何异常，证明它仍会产生用户可见 Mesh；只覆盖用户已经遇到的一个或两个错误码不算完成。
- 只有两类情况允许不发布结果：输入或 Blender context 无效到无法构造任何 Mesh；非预期的程序错误或系统错误。后者必须保留现场信息并直接抛出，禁止伪装成“安全失败”或静默降级。
- 报错提示、红色诊断标记或“source 未改变”不能代替最终可见结果；仅停在提示信息上的实现视为未完成。
