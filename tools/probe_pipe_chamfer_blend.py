...
source = bpy.data.objects.get(os.environ.get("HST_PIPE_PROBE_OBJECT", "Extruded.002"))
if source is None or source.type != "MESH":
    raise RuntimeError("Expected source Mesh Object Extruded.002")
if (len(source.data.vertices), len(source.data.edges), len(source.data.polygons)) < (371, 552, 183):
    raise RuntimeError(
        "Source Mesh Object is smaller than the Extruded.002 regression fixture: "
        f"{len(source.data.vertices)}/{len(source.data.edges)}/{len(source.data.polygons)}"
    )
